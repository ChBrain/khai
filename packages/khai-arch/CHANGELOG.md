...
- d55da1c: Correct `architecture/_schema.yml`'s description: it named a count of typed
...
- c068b0b: Declare the provenance vocabulary: every content file may declare in
...
- 14b6fd7: WIRE: name Plan and Pitch among the engine root types. The kit and model already treat them as engine cargo (`engineMembers` accepts any canon type; `model.md`'s cast group lists both), so this brings the Wire chapter's enumeration in line with what the canon already permits — an engine may now root on a plan or a pitch.

## 0.1.20

### Patch Changes

- a37cb14: Migrate js-yaml to v5 (1/4, arch lane). js-yaml 5 removed the default export and
...
- 046d1a9: read canon frontmatter with js-yaml 4.2.0 instead of gray-matter
