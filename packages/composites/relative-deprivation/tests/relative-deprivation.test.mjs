// The relative-deprivation composite tests only what is composite-specific:
...
describe("relative-deprivation: conforms to the canon", () => {
...
describe("relative-deprivation: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("relative-deprivation");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(3);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_relative_deprivation.md");
  });
...
describe("relative-deprivation: the atoms arrive with the package", () => {
  it("re-exports the two dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["anger", "comparison"]);
...
describe("relative-deprivation: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Relative Deprivation");
    });
  }
