// Entry point for the owing composite. Reads the declarative `khai` manifest from
...
//
// Owing reads the obligation between two persons (F-persona): it wires the three
// engines that carry it at different degrees of exactness (gift, the transfer that binds
// by naming no price; debt, the bond written as a claim one party may call in; money, the
// general equivalent that can discharge any claim at all) and reads them as one thing
// growing more exact. Its three bridges are those degrees: the giving, the owed, and the
// settling twist.
 ...
 * composing a bridge emits the owing root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-owing: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// composite can compose from any of the obligation engines without a second import. The
...
import gift from "@chbrain/khai-engine-gift";
import debt from "@chbrain/khai-engine-debt";
import money from "@chbrain/khai-engine-money";

export const atoms = { gift, debt, money };
