...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-exploration: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from either side of the transition without a
...
// through them.
import curiosity from "@chbrain/khai-engine-curiosity";
import playMode from "@chbrain/khai-engine-play-mode";

export const atoms = { curiosity, "play-mode": playMode };
