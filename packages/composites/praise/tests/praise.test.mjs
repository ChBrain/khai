// The praise composite tests only what is composite-specific: canon
...
describe("praise: conforms to the canon", () => {
...
describe("praise: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("praise");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_praise.md");
  });

  it("hangs all three bridges off the root", () => {
    for (const m of manifest.members.filter((m) => m.parent !== null)) {
      expect(m.parent).toBe("process_praise.md");
    }
    expect(manifest.members.map((m) => m.file)).toEqual([
      "process_praise.md",
      "process_praise_blend.md",
      "process_praise_lift.md",
      "process_praise_reading.md",
    ]);
  });
...
describe("praise: the atoms arrive with the package", () => {
  it("re-exports the three dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["admiration", "elevation", "gratitude"]);
...
describe("praise: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Praise");
    });
  }
