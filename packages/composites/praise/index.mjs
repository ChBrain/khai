// Entry point for the praise composite. Reads the declarative `khai` manifest
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-praise: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
import gratitude from "@chbrain/khai-engine-gratitude";
import elevation from "@chbrain/khai-engine-elevation";
import admiration from "@chbrain/khai-engine-admiration";

export const atoms = { gratitude, elevation, admiration };
