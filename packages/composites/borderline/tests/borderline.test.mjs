// The borderline composite tests only what is composite-specific: canon conformance
...
describe("borderline: conforms to the canon", () => {
...
describe("borderline: manifest", () => {
  ...
  it("carries the four movements as distinct positions", () => {
    const movements = manifest.members.filter((m) => m.parent === "process_borderline.md");
    expect(movements.map((m) => m.file).sort()).toEqual([
      "position_dyscontrol.md",
      "position_emotional_vulnerability.md",
      "position_identity_diffusion.md",
      "position_interpersonal_hypersensitivity.md",
    ]);
...
describe("borderline: the atoms arrive with the package", () => {
...
describe("borderline: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Borderline");
    });
  }
