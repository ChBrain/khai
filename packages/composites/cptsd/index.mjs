// Entry point for the cptsd composite. Reads the declarative `khai` manifest
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-cptsd: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any part of complex PTSD without a second
// import. The dependency graph is the citation graph: these five are declared in
// package.json, and every hard link in the members resolves through them.
import ptsd from "@chbrain/khai-engine-ptsd";
