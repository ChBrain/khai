// Entry point for the love composite. Reads the declarative `khai` manifest from
 ...
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_romantic.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-love: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
import desire from "@chbrain/khai-engine-desire";
import loyalty from "@chbrain/khai-engine-loyalty";

export const atoms = { selfDisclosure, desire, loyalty };
