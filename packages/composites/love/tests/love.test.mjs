// The love composite tests only what is composite-specific: canon conformance
...
describe("love: conforms to the canon", () => {
...
describe("love: manifest", () => {
  it("declares the composite and its root plus three bridges", () => {
    expect(manifest.engine).toBe("love");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    expect(manifest.members.find((m) => m.parent === null).file).toBe("process_love.md");
    const bridges = manifest.members.filter((m) => m.parent !== null).map((m) => m.file);
    expect(bridges).toEqual(["process_romantic.md", "process_abiding.md", "process_fatuous.md"]);
  });
...
describe("love: the atoms arrive with the package", () => {
  it("re-exports the three dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["desire", "loyalty", "selfDisclosure"]);
...
describe("love: compose()", () => {
  const root = "Process: Love";
