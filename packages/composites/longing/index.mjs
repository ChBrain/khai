// Entry point for the longing composite. Reads the declarative `khai` manifest from
...
//
// Longing reads a persona aching at a gap (F-persona): it wires the three distance-of-
// absence engines (loneliness, the absent other; regret, the untaken path; nostalgia,
// the lost past) and reads them as one longing -- a felt distance between the actual and
// the desired, remembered, or possible. Its three bridges are the distances a self
// measures: the other, the path, and the past twist.
 ...
 * composing a bridge emits the longing root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-longing: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// composite can compose from any of the longing engines without a second import. The
...
import loneliness from "@chbrain/khai-engine-loneliness";
import regret from "@chbrain/khai-engine-regret";
import nostalgia from "@chbrain/khai-engine-nostalgia";

export const atoms = { loneliness, regret, nostalgia };
