...
// Every member must wire to something. A composite file that links no atom is
// prose sitting in a package that claims to read over engines -- and the root
// is where it matters most, because the root is what declares the compound.
// The first pass of this composite shipped a root with ZERO hard links and its
// Lever set to a bullet list of its own member files; the manifest promised a
// compound the root never wired. Coverage of the declared atoms is NOT what is
// asserted here: that first pass did link all six, once each, which is why
// "declared equals linked" would have passed it and caught nothing.
describe("hoarding: every member wires into the atoms", () => {
  const declared = new Set(
    Object.keys(JSON.parse(readFileSync(join(pkgDir, "package.json"), "utf8")).dependencies).filter(
      (d) => d.startsWith("@chbrain/khai-engine-"),
    ),
  );
  const atomLinks = (file) => {
    const text = readFileSync(join(pkgDir, file), "utf8");
    return [...text.matchAll(/\]\((@chbrain\/[^/)]+)\/[^)]+\)/g)]
      .map((m) => m[1])
      .filter((pkg) => declared.has(pkg));
  };

  for (const m of manifest.members) {
    it(`${m.file} links at least one declared atom`, () => {
      expect(atomLinks(m.file).length).toBeGreaterThan(0);
    });
  }

  // The root carries the compound, so it should reach more than one of them.
  it("the root reads across several atoms, not one", () => {
    const root = manifest.members.find((x) => x.parent === null).file;
    expect(new Set(atomLinks(root)).size).toBeGreaterThan(1);
  });
});
