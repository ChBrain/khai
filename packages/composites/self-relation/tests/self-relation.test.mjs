// The self-relation composite tests only what is composite-specific: canon
...
describe("self-relation: conforms to the canon", () => {
...
describe("self-relation: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("self-relation");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_self_relation.md");
  });

  it("hangs all three bridges off the root", () => {
    for (const m of manifest.members.filter((m) => m.parent !== null)) {
      expect(m.parent).toBe("process_self_relation.md");
    }
    expect(manifest.members.map((m) => m.file)).toEqual([
      "process_self_relation.md",
      "process_self_relation_modes.md",
      "process_self_relation_fault.md",
      "process_self_relation_weave.md",
    ]);
  });
...
describe("self-relation: the atoms arrive with the package", () => {
  it("re-exports the three dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["authenticity", "self-compassion", "self-esteem"]);
...
describe("self-relation: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Self-Relation");
    });
  }
