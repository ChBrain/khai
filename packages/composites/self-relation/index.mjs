// Entry point for the self-relation composite. Reads the declarative `khai`
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-self-relation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any mode of the family without a second
...
import selfCompassion from "@chbrain/khai-engine-self-compassion";
import authenticity from "@chbrain/khai-engine-authenticity";
