// The moral-account composite tests only what is composite-specific: canon
...
describe("moral-account: conforms to the canon", () => {
...
describe("moral-account: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("moral-account");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_moral_account.md");
  });
...
describe("moral-account: the atoms arrive with the package", () => {
  it("re-exports the four dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["betrayal", "forgiveness", "guilt", "repair"]);
...
describe("moral-account: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Moral Account");
    });
  }
