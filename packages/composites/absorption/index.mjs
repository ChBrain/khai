...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-absorption: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any link of the chain without a second
...
import flow from "@chbrain/khai-engine-flow";
import timePerception from "@chbrain/khai-engine-time-perception";
import attention from "@chbrain/khai-engine-attention";

export const atoms = { flow, "time-perception": timePerception, attention };
