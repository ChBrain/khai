// Entry point for the love-hate composite. Reads the declarative `khai`
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-love-hate: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from either pole of the ambivalence without a second
...
import love from "@chbrain/khai-composite-love";
import hate from "@chbrain/khai-composite-hate";

export const atoms = { love, hate };
