// The rapport composite tests only what is composite-specific: canon conformance
...
describe("rapport: conforms to the canon", () => {
...
describe("rapport: manifest", () => {
  it("declares the composite and its root plus three bridges", () => {
    expect(manifest.engine).toBe("rapport");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    expect(manifest.members.find((m) => m.parent === null).file).toBe("process_rapport.md");
    const bridges = manifest.members.filter((m) => m.parent !== null).map((m) => m.file);
    expect(bridges).toEqual(["process_meshing.md", "process_warming.md", "process_veneer.md"]);
  });
...
describe("rapport: the atoms arrive with the package", () => {
  it("re-exports the three dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["contagion", "grounding", "selfDisclosure"]);
...
describe("rapport: compose()", () => {
  const root = "Process: Rapport";
