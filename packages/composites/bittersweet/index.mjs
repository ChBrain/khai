// Entry point for the bittersweet composite. Reads the declarative `khai`
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-bittersweet: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from either corner of the mix without a second
...
import joy from "@chbrain/khai-engine-joy";
import sadness from "@chbrain/khai-engine-sadness";

export const atoms = { joy, sadness };
