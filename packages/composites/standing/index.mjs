// Entry point for the heritage engine. Reads the declarative `khai` manifest
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-standing: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any altitude of standing without a second
...
import capital from "@chbrain/khai-engine-capital";
import hierarchy from "@chbrain/khai-engine-hierarchy";
import power from "@chbrain/khai-engine-power";
import status from "@chbrain/khai-engine-status";
import statusMove from "@chbrain/khai-engine-status-move";

export const atoms = { capital, hierarchy, power, status, "status-move": statusMove };
