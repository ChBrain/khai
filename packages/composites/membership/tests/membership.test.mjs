// The membership composite tests only what is composite-specific: canon
...
describe("membership: conforms to the canon", () => {
...
describe("membership: manifest", () => {
  it("declares the composite and its root plus three bridges", () => {
    expect(manifest.engine).toBe("membership");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    expect(manifest.members.find((m) => m.parent === null).file).toBe("process_membership.md");
    const bridges = manifest.members.filter((m) => m.parent !== null).map((m) => m.file);
    expect(bridges).toEqual(["process_induction.md", "process_covering.md", "process_tether.md"]);
  });
...
describe("membership: the atoms arrive with the package", () => {
...
describe("membership: compose()", () => {
  const root = "Process: Membership";
