// The intergroup composite tests only what is composite-specific: canon
...
describe("intergroup: conforms to the canon", () => {
...
describe("intergroup: manifest", () => {
  it("declares the composite and its root plus three bridges", () => {
    expect(manifest.engine).toBe("intergroup");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    expect(manifest.members.find((m) => m.parent === null).file).toBe("process_intergroup.md");
    const bridges = manifest.members.filter((m) => m.parent !== null).map((m) => m.file);
    expect(bridges).toEqual(["process_division.md", "process_enmity.md", "process_contact.md"]);
  });
...
describe("intergroup: the atoms arrive with the package", () => {
  it("re-exports the three dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["aggression", "bias", "socialIdentity"]);
...
describe("intergroup: compose()", () => {
  const root = "Process: Intergroup";
