// The adhd composite tests only what is composite-specific: canon conformance
...
describe("adhd: conforms to the canon", () => {
...
describe("adhd: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("adhd");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(5);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_adhd.md");
  });

  // The four are dissociable roads, not one mechanism seen four ways
  // (Sonuga-Barke's dual pathway, plus default-mode interference and the
  // inverse case). A movement quietly dropped or merged would lose the claim
  ...
  it("carries the four movements as distinct positions", () => {
    const movements = manifest.members.filter((m) => m.parent === "process_adhd.md");
    expect(movements.map((m) => m.file).sort()).toEqual([
      "position_behavioral_inhibition.md",
      "position_default_intrusion.md",
      "position_delay_aversion.md",
      "position_hyperfocus.md",
    ]);
...
describe("adhd: the atoms arrive with the package", () => {
