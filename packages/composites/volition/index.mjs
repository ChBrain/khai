...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-volition: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any phase of the crossing without a second
...
import goal from "@chbrain/khai-engine-goal";
import implementationIntention from "@chbrain/khai-engine-implementation-intention";
import executiveFunction from "@chbrain/khai-engine-executive-function";

export const atoms = {
  goal,
  "implementation-intention": implementationIntention,
