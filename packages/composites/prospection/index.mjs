// Entry point for the prospection composite. Reads the declarative `khai` manifest from
...
//
// Prospection reads a persona pulled by the unassured future (F-persona): it wires the
// two future-facing engines (hope, the resting slant across expected valence and felt
// agency; anxiety, the anticipatory apprehension a looming outcome stirs) and reads them
// as one prospective faculty. Its three bridges are the shapes of that facing: the slant,
// the looming, and the reach twist.
 ...
 * composing a bridge emits the prospection root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-prospection: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// composite can compose from either future-facing engine without a second import. The
...
import hope from "@chbrain/khai-engine-hope";
import anxiety from "@chbrain/khai-engine-anxiety";

export const atoms = { hope, anxiety };
