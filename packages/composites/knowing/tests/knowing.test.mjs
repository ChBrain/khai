// The knowing composite tests only what is composite-specific: canon
...
describe("knowing: conforms to the canon", () => {
...
describe("knowing: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("knowing");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_knowing.md");
  });

  it("hangs all three bridges off the root", () => {
    for (const m of manifest.members.filter((m) => m.parent !== null)) {
      expect(m.parent).toBe("process_knowing.md");
    }
    expect(manifest.members.map((m) => m.file)).toEqual([
      "process_knowing.md",
      "process_knowing_sort.md",
      "process_knowing_turn.md",
      "process_knowing_arc.md",
    ]);
  });
...
describe("knowing: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Knowing");
    });
  }
