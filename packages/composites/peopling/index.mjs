// Entry point for the peopling composite. Reads the declarative `khai` manifest from
...
//
// Peopling reads a place by the presence in it (F-place, on Shown): it wires the three
// engines that read a place through its occupants (legibility, the image it can be held
// as; social-time, the rhythm it keeps; street-life, the occupation it hosts) and reads
// them as one graded population -- sorted not by how many are there but by what would
// have kept them away. Its three bridges run up that grade: the errand, the round, and
// the loitering twist, the only presence a place cannot compel.
 ...
 * composing a bridge emits the peopling root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-peopling: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// composite can compose from any of the place engines without a second import. The
...
import legibility from "@chbrain/khai-engine-legibility";
import socialTime from "@chbrain/khai-engine-social-time";
import streetLife from "@chbrain/khai-engine-street-life";

export const atoms = {
  legibility,
  "social-time": socialTime,
