...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-tragic: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any part of the tragic without a second
...
import grief from "@chbrain/khai-engine-grief";
import empathy from "@chbrain/khai-engine-empathy";
import reversal from "@chbrain/khai-engine-reversal";
