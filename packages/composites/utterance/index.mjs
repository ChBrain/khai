// Entry point for the utterance composite. Reads the declarative `khai` manifest from
...
//
// Utterance reads what a saying carries beyond its words (F-persona): it wires the four
// freight engines (speech-act, the act performed; implicature, the meaning never stated;
// register and tone, the variety and affective key it arrives in) and reads them as one
// cargo. Its three bridges are the parts of that freight: the deed, the unsaid, and the
// manner twist.
 ...
 * composing a bridge emits the utterance root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-utterance: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// composite can compose from any of the freight engines without a second import. The
...
import implicature from "@chbrain/khai-engine-implicature";
import register from "@chbrain/khai-engine-register";
import tone from "@chbrain/khai-engine-tone";
