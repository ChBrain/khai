// Entry point for the comic composite. Reads the declarative `khai` manifest
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-comic: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from any part of the comic without a second
...
import humor from "@chbrain/khai-engine-humor";
import emotion from "@chbrain/khai-engine-emotion";
