// The outdone composite tests only what is composite-specific: canon
...
describe("outdone: conforms to the canon", () => {
...
describe("outdone: manifest", () => {
  it("declares the composite and its root", () => {
    expect(manifest.engine).toBe("outdone");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_outdone.md");
  });

  it("hangs all three bridges off the root", () => {
    for (const m of manifest.members.filter((m) => m.parent !== null)) {
      expect(m.parent).toBe("process_outdone.md");
    }
    expect(manifest.members.map((m) => m.file)).toEqual([
      "process_outdone.md",
      "process_outdone_sort.md",
      "process_outdone_turn.md",
      "process_outdone_blend.md",
    ]);
  });
...
describe("outdone: the atoms arrive with the package", () => {
  it("re-exports the two dependency engines", () => {
    expect(Object.keys(atoms).sort()).toEqual(["admiration", "envy"]);
...
describe("outdone: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Outdone");
    });
  }
