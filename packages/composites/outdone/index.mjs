// Entry point for the outdone composite. Reads the declarative `khai` manifest
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-outdone: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// import. The dependency graph is the citation graph: these two are declared in
// package.json, and every hard link in the members resolves through them.
import admiration from "@chbrain/khai-engine-admiration";
import envy from "@chbrain/khai-engine-envy";

export const atoms = { admiration, envy };
