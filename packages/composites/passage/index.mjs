...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-passage: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// the composite can compose from either clock of the passage without a second
// import. The dependency graph is the citation graph: these two are declared
// in package.json, and every hard link in the members resolves through them.
import ritual from "@chbrain/khai-engine-ritual";
import liminality from "@chbrain/khai-engine-liminality";

export const atoms = { ritual, liminality };
