// Entry point for the elsewhere composite. Reads the declarative `khai` manifest from
...
//
// Elsewhere reads the mind in a place it is not (F-persona): it wires the three engines
// that put it there (imagination, the scene built on purpose and quarantined from belief;
// transportation, the story world entered with the guard lowered by the entering; memory,
// the past assembled from a trace and returned as a report) and reads them as one shape
// -- the same recombining machinery at three settings of the guard. Its three bridges are
// those settings: the conjured, the borrowed, and the rebuilt twist.
 ...
 * composing a bridge emits the elsewhere root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-elsewhere: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
// composite can compose from any of the constructive engines without a second import. The
...
import imagination from "@chbrain/khai-engine-imagination";
import transportation from "@chbrain/khai-engine-transportation";
import memory from "@chbrain/khai-engine-memory";

export const atoms = { imagination, transportation, memory };
