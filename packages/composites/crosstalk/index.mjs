// Entry point for the crosstalk composite. Reads the declarative `khai`
 ...
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_loop.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-composite-crosstalk: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
...
import framing from "@chbrain/khai-engine-framing";
import accommodation from "@chbrain/khai-engine-accommodation";

export const atoms = { grounding, framing, accommodation };
