...
correct or optimal: it is about what the participant valued. Accept short
answers; the lens is often quick.

Probe if empty: "Is there anything from the period, however small, that
you would want to keep or repeat?"
...
Probe if empty: "Was there a moment where something surprised you or
clicked into place?"
...
Probe if empty: "Was there a point where you felt blocked or slowed down
...
What the participant wishes had existed or been different: a capability,
a process, a relationship, a resource. Unlike Lacked (which is about
what constrained this period), Longed for can reach beyond the period to
future possibilities.

Probe if empty: "If you could change one thing about how the next period
runs, what would it be?"

## Output format

Compile responses into a document with this structure:

```
# Retrospective: 4 L's
Period: [the period the participant named]
...
- **One probe per lens.** More than one follow-up shifts from facilitation
  to interrogation.
