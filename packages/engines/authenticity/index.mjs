// Entry point for the authenticity engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. Authenticity is
// multi-type: a process root over the three components (process), plus the
// trait authenticity (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_congruence.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-authenticity: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
