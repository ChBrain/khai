// Entry point for the virtue engine. Reads the declarative `khai` manifest
...
//
// Virtue is a mixed-type tree: a process root (process_virtue, the firing) and
// fourteen position leaves (the seven axes, two poles each). The canon owns the
// tree shape, so this loader pulls the composition chains from @chbrain/khai-arch
// rather than re-deriving them here; each member validates against its own type.
...
/**
 * Assemble the ready-to-use instruction set for one pole of the engine: the
 * whole chain from the firing root down to that pole, root first, bodies only.
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-virtue: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
