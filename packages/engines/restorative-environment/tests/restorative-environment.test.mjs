...
describe("restorative-environment: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("restorative-environment: manifest", () => {
  it("declares the restorative-environment place engine", () => {
    expect(manifest.engine).toBe("restorative-environment");
    expect(manifest.type).toBe("place");
    expect(manifest.anchor).toBe("place_restorative_environment.md");
...
describe("restorative-environment: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-quality" })).toThrow();
  });
