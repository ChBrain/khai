// Entry point for the dark-triad engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one dark-triad trait: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * trait upward, so composing a trait emits the shared dark core with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "position_narcissism.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-dark-triad: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
