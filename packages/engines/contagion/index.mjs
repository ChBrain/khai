// Entry point for the contagion engine. Reads the declarative `khai` manifest
...
//
// Contagion is a process tree: a root (contagion) and its two leaves (mimicry,
// convergence). The canon owns the tree shape, so this loader pulls the
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_mimicry.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-contagion: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
