// Entry point for the warning engine. Reads the declarative `khai` manifest from
...
//
// Warning is a process tree: a root (warning) declares the signal issued ahead of
// a harm, four movement heads carry it (the issuing, the receiving, the
// repetition, the aftermath), and each leaf is one form. The canon owns the tree
// shape, so this loader pulls the composition chains from @chbrain/khai-arch
// rather than re-deriving them here.
//
// The four run in order for one signal and loop across many, since the aftermath
// sets the volume the next warning is heard at -- so a play composes the members
// it needs rather than stepping the tree.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_recalibration.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-warning: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
