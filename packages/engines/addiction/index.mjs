// Entry point for the addiction engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one addiction drive: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * compulsion dynamic upward, so composing a drive emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_craving.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-addiction: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
