// Entry point for the attachment engine. Reads the declarative `khai` manifest
...
//
// Like stress and emotion, attachment is a process tree: a root (attachment)
// and its leaves (secure, anxious, avoidant, disorganized). The canon owns the
 ...
 * deeper member carries the shallower ones upward, so composing a strategy emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_avoidant.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-attachment: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
