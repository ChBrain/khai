// Entry point for the coping engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. coping is
// a process engine: a process root over the three forms of the appraisal-driven repertoire,
// appraisal, problem-focused, and avoidant; each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_appraisal.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-coping: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
