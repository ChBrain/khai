// Entry point for the gratitude engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one gratitude mode: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * grateful turn upward, so composing a mode emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_benefaction.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-gratitude: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
