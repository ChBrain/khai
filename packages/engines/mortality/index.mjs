// Entry point for the mortality engine. Reads the declarative `khai` manifest
...
/** Each expression, body only, keyed by name (e.g. worldview, acceptance). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-mortality: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
