// The heirloom engine tests only what is heirloom-specific: that the package
...
describe("heirloom: conforms to the canon", () => {
...
describe("heirloom: manifest", () => {
  it("declares the heirloom engine rooted on a piece", () => {
    expect(manifest.engine).toBe("heirloom");
    expect(manifest.type).toBe("piece");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("piece_heirloom.md");
  });
...
describe("heirloom: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Piece: Heirloom");
    });
  }
