// The desire engine tests only what is desire-specific: that the package
...
// --- self-conformance: desire certifies itself against the canon ---------
describe("desire: conforms to the canon", () => {
...
// --- manifest contract ----------------------------------------------------
describe("desire: manifest", () => {
  it("declares the desire position engine", () => {
    expect(manifest.engine).toBe("desire");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_desire.md");
...
describe("desire: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-desire" })).toThrow();
  });
