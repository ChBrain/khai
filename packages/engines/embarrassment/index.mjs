// Entry point for the embarrassment engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. Embarrassment is
// multi-type: a process root over the four forms (process), plus the trait
// embarrassability (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_faux_pas.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-embarrassment: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
