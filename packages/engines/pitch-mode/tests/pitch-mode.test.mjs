...
describe("pitch-mode: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("pitch-mode: manifest", () => {
  it("declares the pitch-mode position engine", () => {
    expect(manifest.engine).toBe("pitch-mode");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_pitch_mode.md");
...
describe("pitch-mode: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-pitch" })).toThrow();
  });
