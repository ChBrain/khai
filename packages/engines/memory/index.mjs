// Entry point for the memory engine. Reads the declarative `khai` manifest from
...
/**
 * Assemble the instruction set for one kind of remembering: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * reconstruction dynamic upward, so composing a kind emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_episodic.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-memory: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
