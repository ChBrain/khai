// Entry point for the achievement-motive engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. hope_of_success, fear_of_failure). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-achievement-motive: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
