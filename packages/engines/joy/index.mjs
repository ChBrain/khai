// Entry point for the joy engine. Reads the declarative `khai` manifest from
...
// @chbrain/khai-arch rather than re-deriving them here. Joy is multi-type: a
// process root over the four forms (process), plus the trait sanguine
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_pleasure.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-joy: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
