// Entry point for the complaint engine. The engine is content: seventeen
...
//
// The tree is two deep. The root is the grievance in the channel; four
// movements hang off it in the order a case runs (the filing, the procedure,
// the cost, the outcome); three forms hang off each movement. chains is keyed
// by true leaf only, so the root and the four movement heads are not
// composable on their own -- they are carried upward by whatever hangs below.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_single_case.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-complaint: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
