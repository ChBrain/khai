// Entry point for the speech-act engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one speech act: the whole chain from the
 * root down to that leaf, root first, bodies only. The leaf carries the act
 * upward, so composing an act emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_directive.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-speech-act: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
