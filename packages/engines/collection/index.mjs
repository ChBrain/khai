// Entry point for the collection engine. Reads the declarative `khai` manifest
...
//
// Collection is a piece engine: the anchor is the object -- the series itself --
// and the members hang flat beneath it. One position names who keeps it, and
// three processes name what it does: the hunting, the completing, the dispersal.
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-collection: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
