...
describe("psychological-safety: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("psychological-safety: manifest", () => {
  it("declares the psychological-safety place engine", () => {
    expect(manifest.engine).toBe("psychological-safety");
    expect(manifest.type).toBe("place");
    expect(manifest.anchor).toBe("place_psychological_safety.md");
...
describe("psychological-safety: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-climate" })).toThrow();
  });
