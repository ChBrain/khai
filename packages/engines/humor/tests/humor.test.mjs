...
describe("humor: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("humor: manifest", () => {
  it("declares the humor position engine", () => {
    expect(manifest.engine).toBe("humor");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_humor.md");
...
describe("humor: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-humor" })).toThrow();
  });
