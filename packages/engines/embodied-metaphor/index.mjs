// Entry point for the embodied-metaphor engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. temperature, verticality). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-embodied-metaphor: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
