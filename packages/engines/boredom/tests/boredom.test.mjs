// The boredom engine tests only what is boredom-specific: that the package
...
describe("boredom: conforms to the canon", () => {
...
describe("boredom: manifest", () => {
  it("declares the boredom engine and its anchor", () => {
    expect(manifest.engine).toBe("boredom");
    expect(manifest.members).toHaveLength(5);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_boredom.md");
  });
...
describe("boredom: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Boredom");
    });
  }
