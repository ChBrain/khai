// Entry point for the compassion engine. Reads the declarative `khai` manifest
...
// @chbrain/khai-arch rather than re-deriving them here. Compassion is
// multi-type: a process root over the two faces and two shadows (process),
// plus the trait tenderhearted (position); each member validates against its
// own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_tenderness.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-compassion: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
