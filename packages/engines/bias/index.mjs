// Entry point for the bias engine. Reads the declarative `khai` manifest
...
/** Each expression, body only, keyed by name (e.g. self, coherence, ease). */
export const expressions = Object.fromEntries(
  Object.keys(manifest.expressions).map((name) => [name, body(raw[name])]),
);

/**
 * Assemble the instruction set for a persona holding one tilt: the anchor
 * followed by the chosen family. The expression carries the anchor upward, per
 * the engine's wiring.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-bias: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
