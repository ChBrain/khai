// Entry point for the hatred engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. hatred is
// a process engine: a process root over the three components of the sentiment,
// repulsion, malice, and essentializing; each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_repulsion.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-hatred: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
