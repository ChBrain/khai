// Entry point for the transportation engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. Transportation is
// multi-type: a process root over the three components (process), plus the
// trait transportability (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_imagery.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-transportation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
