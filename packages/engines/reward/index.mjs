// Entry point for the reward engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one reward mode: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * valuation dynamic upward, so composing a mode emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_immediate.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-reward: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
