// Entry point for the structural-model engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. id, ego, superego). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-structural-model: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
