...
describe("curiosity: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("curiosity: manifest", () => {
  it("declares the curiosity position engine", () => {
    expect(manifest.engine).toBe("curiosity");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_curiosity.md");
...
describe("curiosity: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-curiosity" })).toThrow();
  });
