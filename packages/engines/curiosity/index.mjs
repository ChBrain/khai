// Entry point for the curiosity engine. Reads the declarative `khai` manifest
...
/** Each expression, body only, keyed by name (e.g. epistemic, perceptual). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-curiosity: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
