// Entry point for the photograph engine. Reads the declarative `khai` manifest
...
//
// Photograph is a piece engine: the anchor is the object -- the image itself --
// and the members hang flat beneath it. One position names who is in it, and
// three processes name what it does: the taking, the showing, the outliving.
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-photograph: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
