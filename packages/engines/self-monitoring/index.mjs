// Entry point for the self-monitoring engine. Reads the declarative `khai`
// manifest from package.json (the single source of truth for how this
// engine wires) and assembles the markdown positions into a ready-to-use
// instruction set.
//
...
/** Each expression, body only, keyed by name (e.g. scanning, tuning, consistency). */
 ...
 * expression: the anchor followed by the chosen expression. The expression
 * carries the anchor upward, per the engine's wiring.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-self-monitoring: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
