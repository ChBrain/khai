// Entry point for the expertise engine. Reads the declarative `khai` manifest from
...
//
// Expertise is a process tree: a root (expertise) declares a capacity moving out
// of deliberation and into the hands, three movement heads carry the climb, what
// builds it, and what it costs (the stages, the making, the tacit), and each leaf
// is one stage or form. The canon owns the tree shape, so this loader pulls the
// composition chains from @chbrain/khai-arch rather than re-deriving them here.
//
// Only the stages movement is ordered. The making and the tacit are conditions
// that run alongside the climb rather than after it -- a persona can be plateaued
// at the competent stage, or blind-spotted while still merely proficient -- so a
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_plateau.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-expertise: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
