// Entry point for the lottery engine. Reads the declarative `khai` manifest from
...
//
// Lottery is a process tree: a root (lottery) declares allocation by chance,
// four movement heads carry it (the impasse, the drawing, the verdict, the
// filling), and each leaf is one form. The canon owns the tree shape, so this
...
// filling supplies what the next draw is entered on -- so a play composes the
// members it needs rather than stepping the tree.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_no_reasons.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-lottery: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
