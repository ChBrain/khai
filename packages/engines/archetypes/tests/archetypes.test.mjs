// The archetypes engine tests only what is archetypes-specific: that the
...
describe("archetypes: conforms to the canon", () => {
...
describe("archetypes: manifest", () => {
  it("declares the archetypes engine and its anchor", () => {
    expect(manifest.engine).toBe("archetypes");
    expect(manifest.members).toHaveLength(8);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("persona_archetype.md");
  });
...
describe("archetypes: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Persona: Archetype");
    });
  }
