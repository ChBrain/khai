// Entry point for the loneliness engine. Reads the declarative `khai` manifest
...
//
// Loneliness is a process tree: a root (loneliness) and its leaves (reaching,
// withdrawal, numbness, reconnected). The canon owns the tree shape, so this
 ...
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_withdrawal.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-loneliness: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
