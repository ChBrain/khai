// Entry point for the self-efficacy engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. mastery, vicarious). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-self-efficacy: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
