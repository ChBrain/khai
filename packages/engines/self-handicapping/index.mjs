// Entry point for the self-handicapping engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. self-handicapping is
// multi-type: a process root over the two handicap forms (process), plus the
// standing handicapper disposition (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_acquired.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-self-handicapping: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
