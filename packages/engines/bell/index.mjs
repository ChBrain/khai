// Entry point for the bell engine. Reads the declarative `khai` manifest from
...
//
// Bell is a piece engine: the anchor is the object -- the cast instrument in the
// tower -- and the members hang flat beneath it. One position names who holds
// the rope, and three processes name what it does: the casting, the ringing,
// the earshot.
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-bell: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
