...
describe("total-institution: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("total-institution: manifest", () => {
  it("declares the total-institution place engine", () => {
    expect(manifest.engine).toBe("total-institution");
    expect(manifest.type).toBe("place");
    expect(manifest.anchor).toBe("place_total_institution.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["barrier", "batch", "mortification"]);
  });
...
describe("total-institution: compose()", () => {
