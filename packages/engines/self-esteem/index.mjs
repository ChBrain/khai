// Entry point for the self-esteem engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. high, low, contingent). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-self-esteem: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
