// The mood engine tests only what is mood-specific: that the package
...
// detection - a dropped chapter, an invented Owner key, an undeclared extension -
// is proved in khai-tests against its own fixture, not re-proved here through
// mood's files.
...
// --- self-conformance: mood certifies itself against the canon ------------
describe("mood: conforms to the canon", () => {
...
// --- manifest contract ----------------------------------------------------
describe("mood: manifest", () => {
  it("declares the mood position engine", () => {
    expect(manifest.engine).toBe("mood");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_mood.md");
...
describe("mood: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-mood" })).toThrow();
  });
