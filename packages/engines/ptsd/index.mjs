// Entry point for the ptsd engine. Reads the declarative `khai` manifest from
...
//
// Like stress and attachment, ptsd is a process tree: a root (ptsd) and its
// leaves (reexperiencing, evasion, estrangement, hyperarousal). The canon owns
 ...
 * deeper member carries the shallower ones upward, so composing a cluster emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_reexperiencing.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-ptsd: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
