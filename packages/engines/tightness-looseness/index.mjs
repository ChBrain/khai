// Entry point for the tightness-looseness engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. tight, loose). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-tightness-looseness: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
