// Entry point for the interpreting engine. Reads the declarative `khai` manifest
...
//
// Interpreting is a process tree: a root (interpreting) declares a conversation
// conducted through a third person, four movement heads carry it (the summoning,
// the relaying, the coordinating, the invisibility), and each leaf is one form.
// The canon owns the tree shape, so this loader pulls the composition chains
// from @chbrain/khai-arch rather than re-deriving them here.
//
// The four are concurrent rather than sequential: the summoning sets the terms
// once, and the relaying, the coordinating, and the invisibility run together on
// every turn -- so a play composes the members it needs rather than stepping the
// tree.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_side_talk.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-interpreting: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
