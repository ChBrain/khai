// Entry point for the sadness engine. Reads the declarative `khai` manifest
...
// @chbrain/khai-arch rather than re-deriving them here. Sadness is multi-type:
// a process root over the four forms (process), plus the trait dysphoria
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_sorrow.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-sadness: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
