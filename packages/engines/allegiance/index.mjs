// Entry point for the allegiance engine. Reads the declarative `khai` manifest
...
// @chbrain/khai-arch rather than re-deriving them here. Allegiance is a process
// engine: a root over the three forms of affective disposition; each member
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_rooting.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-allegiance: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
