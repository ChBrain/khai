// The aggression engine tests only what is aggression-specific: that the package
...
describe("aggression: conforms to the canon", () => {
...
describe("aggression: manifest", () => {
  it("declares the aggression engine and its anchor", () => {
    expect(manifest.engine).toBe("aggression");
    expect(manifest.members).toHaveLength(6);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_aggression.md");
  });
...
describe("aggression: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Aggression");
    });
  }
