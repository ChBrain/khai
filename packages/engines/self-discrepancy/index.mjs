// Entry point for the self-discrepancy engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. self-discrepancy is
// multi-type: a process root over the two discrepancy forms (process), plus the
// standing discrepant profile (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_ideal.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-self-discrepancy: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
