// Entry point for the shame engine. Reads the declarative `khai` manifest
...
//
// Shame is a process tree: a root (shame) and its leaves (guilt, embarrassment,
// humiliation). The canon owns the tree shape, so this loader pulls the
 ...
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_guilt.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-shame: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
