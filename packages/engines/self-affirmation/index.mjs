// Entry point for the self-affirmation engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. self-affirmation is
// a process engine: a process root over the three moments of the restoration,
// affirming, adequacy, and buffering; each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_affirming.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-self-affirmation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
