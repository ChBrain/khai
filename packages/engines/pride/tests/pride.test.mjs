// The pride engine tests only what is pride-specific: that the package
...
// --- self-conformance: pride certifies itself against the canon -----------
describe("pride: conforms to the canon", () => {
...
describe("pride: manifest", () => {
  ...
  it("declares both enforceable wiring altitudes, each at its level", () => {
    ...
    // the root process. Two: the read linking the specific leaf facet.
...
describe("pride: compose()", () => {
  const root = "Process: Pride";

  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first, then the facet`, () => {
  ...
  it("rejects an unknown facet", () => {
    expect(() => compose({ leaf: "process_vanity.md" })).toThrow();
  });
