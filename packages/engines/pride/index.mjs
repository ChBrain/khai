// Entry point for the pride engine. Reads the declarative `khai` manifest
...
//
// Like emotion, pride is a process tree: a root (pride) and its leaves
// (authentic pride, hubristic pride). The canon owns the tree shape, so this
 ...
 * deeper member carries the shallower ones upward, so composing a facet emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_authentic_pride.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-pride: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
