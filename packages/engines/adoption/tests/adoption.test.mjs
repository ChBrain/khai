// The adoption engine tests only what is adoption-specific: that the package
...
describe("adoption: conforms to the canon", () => {
...
describe("adoption: manifest", () => {
  it("declares the adoption engine and its anchor", () => {
    expect(manifest.engine).toBe("adoption");
    expect(manifest.members).toHaveLength(10);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_adopt_idea.md");
  });
...
describe("adoption: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Adopt Idea");
    });
  }
