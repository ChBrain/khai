// Entry point for the regulation engine. Reads the declarative `khai` manifest
...
//
// Like stress and emotion, regulation is a process tree: a root (regulation)
// and its leaves (situation_selection, reappraisal, suppression). The canon
 ...
 * deeper member carries the shallower ones upward, so composing a strategy
 * emits its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_reappraisal.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-regulation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
