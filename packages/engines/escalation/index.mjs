// Entry point for the escalation engine. Reads the declarative `khai` manifest
...
//
// Escalation is a mixed-type tree: a process root (process_escalation, the
// spiral) with two process children (hardening, retaliation) and one position
// child (antagonist). The canon owns the tree shape, so this loader pulls the
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-escalation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
