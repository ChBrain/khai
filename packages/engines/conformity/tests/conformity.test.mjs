// The conformity engine tests only what is conformity-specific: that the package
...
describe("conformity: conforms to the canon", () => {
...
describe("conformity: manifest", () => {
  it("declares the conformity engine and its anchor", () => {
    expect(manifest.engine).toBe("conformity");
    expect(manifest.members).toHaveLength(5);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_conformity.md");
  });
...
describe("conformity: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Conformity");
    });
  }
