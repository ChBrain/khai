...
describe("moral-conviction: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("moral-conviction: manifest", () => {
  it("declares the position engine over the two mandate faces", () => {
    expect(manifest.engine).toBe("moral-conviction");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_moral_conviction.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["intolerance", "mobilization"]);
  });
...
describe("moral-conviction: compose()", () => {
