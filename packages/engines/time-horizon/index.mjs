// Entry point for the time-horizon engine. Reads the declarative `khai` manifest from
// package.json (the single source of truth for how this engine wires) and assembles the
// markdown positions into a ready-to-use instruction set.
//
...
// Time-horizon is the survivable core of the life-course branch: an anchor (a persona's
// resting sense of how much time it has left) over two poles -- expansive and limited.
...
/** Each expression, body only, keyed by name (expansive, limited). */
 ...
 * expression: the anchor followed by the chosen horizon.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-time-horizon: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
