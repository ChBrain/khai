// Entry point for the identity engine. Reads the declarative `khai` manifest
...
/** Each expression, body only, keyed by name (e.g. achieved, diffuse). */
 ...
 * followed by the chosen status. The expression carries the anchor upward, per
 * the engine's wiring.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-identity: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
