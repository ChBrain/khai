...
describe("status: conforms to the canon", () => {
...
describe("status: manifest", () => {
  it("declares the status position engine", () => {
    expect(manifest.engine).toBe("status");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_status.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["contested", "high", "low", "token"]);
  });
...
describe("status: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-status" })).toThrow();
  });
