// Entry point for the register engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one register mode: the whole chain from the
 * root down to that leaf, root first, bodies only. The leaf carries the mode
 * upward, so composing a mode emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "position_shorthand.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-register: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
