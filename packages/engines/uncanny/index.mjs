// Entry point for the uncanny engine. Reads the declarative `khai` manifest
...
//
// Uncanny is a process tree: a root (uncanny) and its leaves (double,
// recurrence, animism, animate, revenant). The canon owns the tree shape, so
 ...
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_double.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-uncanny: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
