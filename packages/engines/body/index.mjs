// Entry point for the body engine. Reads the declarative `khai` manifest from
...
//
// Like emotion, body is a process tree: a root (body) and its leaves (hunger,
// fatigue, pain, arousal, sickness). The canon owns the tree shape, so this
 ...
 * deeper member carries the shallower ones upward, so composing a demand emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_hunger.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-body: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
