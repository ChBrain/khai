// The heritage engine tests only what is heritage-specific: that the package
...
describe("heritage: conforms to the canon", () => {
...
describe("heritage: manifest", () => {
  it("declares the heritage engine and its anchor", () => {
    expect(manifest.engine).toBe("heritage");
    expect(manifest.members).toHaveLength(12);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("position_heritage.md");
  });
...
describe("heritage: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Position: Heritage");
    });
  }
