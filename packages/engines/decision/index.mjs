// Entry point for the decision engine. Reads the declarative `khai` manifest
...
//
// Decision is a process tree: a root (decision) and its leaves (intuitive,
// deliberate, heuristic, satisficing, paralysed). The canon owns the tree shape,
 ...
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_deliberate.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-decision: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
