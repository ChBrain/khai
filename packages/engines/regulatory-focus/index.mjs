// Entry point for the regulatory-focus engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. promotion, prevention). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-regulatory-focus: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
