...
describe("empathy: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("empathy: manifest", () => {
  it("declares the empathy position engine", () => {
    expect(manifest.engine).toBe("empathy");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_empathy.md");
...
describe("empathy: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-an-empathy" })).toThrow();
  });
