// The habit engine tests only what is habit-specific: that the package
...
describe("habit: conforms to the canon", () => {
...
describe("habit: manifest", () => {
  it("declares the habit engine and its anchor", () => {
    expect(manifest.engine).toBe("habit");
    expect(manifest.members).toHaveLength(5);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_habit.md");
  });
