// Entry point for the mindset engine. Reads the declarative `khai` manifest
...
/** Each expression, body only, keyed by name (e.g. fixed, growth). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-mindset: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
