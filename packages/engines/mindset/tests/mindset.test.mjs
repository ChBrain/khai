// The mindset engine tests only what is mindset-specific: that the package
...
// --- self-conformance: mindset certifies itself against the canon --------
describe("mindset: conforms to the canon", () => {
...
// --- manifest contract ----------------------------------------------------
describe("mindset: manifest", () => {
  it("declares the mindset position engine", () => {
    expect(manifest.engine).toBe("mindset");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_mindset.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["fixed", "growth"]);
  });
...
describe("mindset: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-mindset" })).toThrow();
  });
