// Entry point for the attribution engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one attribution reading: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * causal reading upward, so composing a reading emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_disposition.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-attribution: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
