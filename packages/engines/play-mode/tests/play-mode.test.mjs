...
describe("play-mode: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("play-mode: manifest", () => {
  it("declares the play-mode position engine", () => {
    expect(manifest.engine).toBe("play-mode");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_play_mode.md");
...
describe("play-mode: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-mode" })).toThrow();
  });
