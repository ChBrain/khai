// Entry point for the place-attachment engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. rooted, dependence). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-place-attachment: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
