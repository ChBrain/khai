// The loyalty engine tests only what is loyalty-specific: that the package
...
// --- self-conformance: loyalty certifies itself against the canon ---------
describe("loyalty: conforms to the canon", () => {
...
// --- manifest contract ----------------------------------------------------
describe("loyalty: manifest", () => {
  it("declares the loyalty position engine", () => {
    expect(manifest.engine).toBe("loyalty");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_loyalty.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["exit", "voice"]);
  });
...
describe("loyalty: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-loyalty" })).toThrow();
  });
