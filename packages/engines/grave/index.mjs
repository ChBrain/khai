// Entry point for the grave engine. Reads the declarative `khai` manifest
...
//
// Grave is a piece engine: the anchor is the object -- the marked plot itself --
// and the members hang flat beneath it. One position names who holds it, and
// three processes name what it does: the burying, the visiting, the lapsing.
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-grave: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
