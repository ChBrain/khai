// Entry point for the credibility engine. Reads the declarative `khai` manifest
...
//
// Credibility is a process tree: a root (credibility) declares the credit given
// to a first-person account, four movement heads carry it (the report, the
// discount, the missing word, the aftermath), and each leaf is one form. The
...
//
// The four are not a schedule. The missing word can precede any hearing at all,
// and the aftermath compounds across occasions -- so a play composes the members
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_unnamed.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-credibility: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
