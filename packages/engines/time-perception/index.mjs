// Entry point for the time-perception engine. Reads the declarative `khai` manifest
...
//
// Zero runtime dependencies beyond Node ESM. The same loader works for any
...
/** Each expression, body only, keyed by name (e.g. continuous, now, behind, absorbed). */
export const expressions = Object.fromEntries(
  Object.keys(manifest.expressions).map((name) => [name, body(raw[name])]),
);

/**
 * Assemble the instruction set for a persona holding one temporal expression: the anchor
 * followed by the chosen expression. The expression carries the anchor upward, per the
 * engine's wiring.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-time-perception: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
