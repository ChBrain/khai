// The secret engine tests only what is secret-specific: that the package
...
describe("secret: conforms to the canon", () => {
...
describe("secret: manifest", () => {
  it("declares the secret engine rooted on a piece", () => {
    expect(manifest.engine).toBe("secret");
    expect(manifest.type).toBe("piece");
    expect(manifest.members).toHaveLength(5);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("piece_secret.md");
  });
