// Entry point for the anger engine. Reads the declarative `khai` manifest from
...
// @chbrain/khai-arch rather than re-deriving them here. Anger is multi-type: a
// process root over the four forms (process), plus the trait hostility
// (position); each member validates against the chapters of its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_frustration.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-anger: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
