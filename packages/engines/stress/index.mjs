// Entry point for the stress engine. Reads the declarative `khai` manifest
...
//
// Unlike a depth-1 position engine (anchor + expressions), stress is a process
// tree: a root (stress) and its leaves (fight, flight, freeze, fawn). The
 ...
 * deeper member carries the shallower ones upward, so composing a response emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_fight.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-stress: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
