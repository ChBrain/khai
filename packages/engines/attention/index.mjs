// Entry point for the attention engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one mode of attention: the whole chain from
 * the root down to that leaf, root first, bodies only. The leaf carries the
 * selection dynamic upward, so composing a mode emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_selective.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-attention: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
