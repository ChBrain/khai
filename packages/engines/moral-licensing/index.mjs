// Entry point for the moral-licensing engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. moral-licensing is
// a process engine: a process root over the two directions of the moral balance,
// indulgence and cleansing; each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_indulgence.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-moral-licensing: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
