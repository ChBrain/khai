// Entry point for the ostracism engine. Reads the declarative `khai` manifest
...
// @chbrain/khai-arch rather than re-deriving them here. Ostracism is a process
// engine: a root over the three stages of Williams' temporal need-threat model;
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_reflexive.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-ostracism: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
