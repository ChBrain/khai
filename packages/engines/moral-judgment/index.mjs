// Entry point for the moral-judgment engine. Reads the declarative `khai`
// manifest from package.json (the single source of truth for how this engine
// wires) and assembles the markdown positions into a ready-to-use instruction set.
//
...
/** Each expression, body only, keyed by name (e.g. care, fairness). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-moral-judgment: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
