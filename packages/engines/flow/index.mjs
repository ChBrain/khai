// Entry point for the flow engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one leaf: the whole chain from the root
 * down to that leaf, root first, bodies only. The leaf carries the root
 * upward, so composing a leaf emits the root with it.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-flow: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
