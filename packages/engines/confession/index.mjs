// Entry point for the confession engine. Reads the declarative `khai` manifest
...
//
// Confession is a process tree: a root (confession) declares telling under a
// relation of authority, four movement heads carry the operation (the demand, the
// composing, the listener, the relief), and each leaf is one form. The canon owns
// the tree shape, so this loader pulls the composition chains from
// @chbrain/khai-arch rather than re-deriving them here.
//
// The four run in order within a single telling and loop across many, since the
// relief fades and the persona returns -- so a play composes the members it needs
// rather than treating the tree as a one-way sequence.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_detail.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-confession: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
