// Entry point for the palimpsest engine. Reads the declarative `khai` manifest
...
// @chbrain/khai-arch; each member validates against its own khai type (pitch).
 ...
 * whole chain from the anchor down to that leaf, anchor first, bodies only. The
 * leaf carries the anchor upward, so composing a leaf emits the anchor with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "pitch_parody.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-palimpsest: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
