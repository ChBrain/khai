...
describe("capital: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("capital: manifest", () => {
  it("declares the capital position engine", () => {
    expect(manifest.engine).toBe("capital");
...
describe("capital: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-capital" })).toThrow();
  });
