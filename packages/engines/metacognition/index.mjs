// Entry point for the metacognition engine. Reads the declarative `khai`
...
// from @chbrain/khai-arch rather than re-deriving them here. Metacognition is
// multi-type: a process root over the three forms (process), plus the trait
// metacognitive skill (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_monitoring.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-metacognition: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
