// The jealousy engine tests only what is jealousy-specific: that the package
...
describe("jealousy: conforms to the canon", () => {
...
describe("jealousy: manifest", () => {
  it("declares the jealousy engine rooted on a process", () => {
    expect(manifest.engine).toBe("jealousy");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(4);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_jealousy.md");
  });
...
describe("jealousy: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Jealousy");
    });
  }
