// Entry point for the interest engine. Reads the declarative `khai` manifest
...
// @chbrain/khai-arch rather than re-deriving them here. Interest is multi-type:
// a process root over the three forms (process), plus the trait individual
// interest (position); each member validates against its own khai type.
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_interest_novel.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-interest: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
