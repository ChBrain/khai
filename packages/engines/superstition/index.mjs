// Entry point for the superstition engine. Reads the declarative `khai` manifest
...
/** Each expression, body only, keyed by name (e.g. contagion, omen). */
 ...
 * followed by the chosen link. The expression carries the anchor upward, per the
 * engine's wiring.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-superstition: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
