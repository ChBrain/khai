// Entry point for the implicature engine. Reads the declarative `khai` manifest
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_maxims.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-implicature: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
