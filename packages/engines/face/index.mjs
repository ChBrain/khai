// Entry point for the face engine. Reads the declarative `khai` manifest from
...
/**
 * Assemble the instruction set for one face strategy: the whole chain from the
 * root down to that leaf, root first, bodies only. The leaf carries the strategy
 * upward, so composing a strategy emits the root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_deference.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-face: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
