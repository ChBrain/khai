// Entry point for the motivation engine. Reads the declarative `khai` manifest
...
//
// Like stress, emotion, and attachment, motivation is a process tree: a root
// (motivation) and its leaves (intrinsic, identified, introjected, external,
// amotivated). The canon owns the tree shape, so this loader pulls the
 ...
 * deeper member carries the shallower ones upward, so composing a style emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_intrinsic.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-motivation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
