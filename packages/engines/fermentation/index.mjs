// Entry point for the fermentation engine. The engine is content: seventeen
...
//
// The tree is two deep. The root is directed spoilage; four movements hang off
// it in the order a vessel runs (the substrate, the setting, the succession,
// the turn); three forms hang off each movement. chains is keyed by true leaf
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_hurdles.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-fermentation: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
