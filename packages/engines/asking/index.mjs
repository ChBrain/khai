// Entry point for the asking engine. Reads the declarative `khai` manifest from
...
//
// Asking is a process tree: a root (asking) declares the request for
// discretionary help, four movement heads carry it (the reluctance, the
// misestimate, the asking, the granting), and each leaf is one form. The canon
// owns the tree shape, so this loader pulls the composition chains from
// @chbrain/khai-arch rather than re-deriving them here.
//
// The four run in order within a single request and loop across many, since the
// granting does not update the asker and the next ask starts from the same
// number -- so a play composes the members it needs rather than stepping the tree.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_ask_unmade.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-asking: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
