// Entry point for the ambivalence engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. potential, felt). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-ambivalence: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
