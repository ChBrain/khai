// The morphology engine tests only what is morphology-specific: that the
...
describe("morphology: conforms to the canon", () => {
...
describe("morphology: manifest", () => {
  it("declares the morphology engine and its anchor", () => {
    expect(manifest.engine).toBe("morphology");
    expect(manifest.members).toHaveLength(7);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("play_folktale.md");
  });
...
describe("morphology: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Play: The Folktale");
    });
  }
