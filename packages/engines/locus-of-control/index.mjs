// Entry point for the locus-of-control engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. internal, external). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-locus-of-control: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
