// The locus-of-control engine tests only what is locus-of-control-specific:
...
// not re-proved here through locus-of-control's files.
...
// --- self-conformance: locus-of-control certifies itself against the canon
describe("locus-of-control: conforms to the canon", () => {
...
// --- manifest contract ----------------------------------------------------
describe("locus-of-control: manifest", () => {
  it("declares the locus-of-control position engine", () => {
    expect(manifest.engine).toBe("locus-of-control");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_locus_of_control.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["external", "internal"]);
  });
  ...
  it("declares both enforceable wiring altitudes, each at its level", () => {
    ...
    // chapter (links the anchor), and the per-persona expectancy under Projection.
    // Both are the structural floor, so both are declared `fail`.
...
describe("locus-of-control: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-locus" })).toThrow();
  });
