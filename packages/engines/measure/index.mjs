// Entry point for the measure engine. Reads the declarative `khai` manifest from
...
//
// Measure is a process tree: a root (measure) declares a quality handled as a
// quantity, four movement heads carry what follows (the cut, the warrant, the
// turn, the territory), and each leaf is one form. The canon owns the tree shape,
// so this loader pulls the composition chains from @chbrain/khai-arch rather than
// re-deriving them here.
//
// The four movements run together rather than in sequence: once the cut has been
// made the other three are already operating, and a play composes the members it
// needs rather than walking the tree in order.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_goodhart.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-measure: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
