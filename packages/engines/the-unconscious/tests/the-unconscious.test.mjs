...
describe("the-unconscious: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("the-unconscious: manifest", () => {
  it("declares the position engine over the three kinds", () => {
    expect(manifest.engine).toBe("the-unconscious");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_the_unconscious.md");
...
describe("the-unconscious: compose()", () => {
  for (const name of Object.keys(expressions)) {
    it(`composes ${name}: anchor first, then the kind`, () => {
  ...
  it("rejects an unknown kind", () => {
    expect(() => compose({ expression: "not-a-kind" })).toThrow();
  });
