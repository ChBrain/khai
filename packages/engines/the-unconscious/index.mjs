// Entry point for the the-unconscious engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. dynamic, adaptive, preconscious). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-the-unconscious: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
