// Entry point for the capitalization engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. active_constructive). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-capitalization: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
