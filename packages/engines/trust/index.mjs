// Entry point for the trust engine. Reads the declarative `khai` manifest
...
//
// Like emotion, trust is a process tree: a root (trust) and its leaves (extend,
// test, withhold, betrayed, repair). The canon owns the tree shape, so this
 ...
 * deeper member carries the shallower ones upward, so composing a move emits
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_extend.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-trust: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
