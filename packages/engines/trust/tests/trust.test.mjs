// The trust engine tests only what is trust-specific: that the package
...
// --- self-conformance: trust certifies itself against the canon -----------
describe("trust: conforms to the canon", () => {
...
describe("trust: manifest", () => {
  it("declares the trust process engine", () => {
    expect(manifest.engine).toBe("trust");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(6);
    expect(manifest.members.find((m) => m.parent === null).file).toBe("process_trust.md");
  });

  it("declares both enforceable wiring altitudes, each at its level", () => {
    // The engine owns the contract; the kit enforces it. One: the law linking
    // the root process. Two: the read linking the specific leaf move.
...
describe("trust: compose()", () => {
  const root = "Process: Trust";

  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: root first, then the move`, () => {
  ...
  it("rejects an unknown move", () => {
    expect(() => compose({ leaf: "process_distrust.md" })).toThrow();
  });
