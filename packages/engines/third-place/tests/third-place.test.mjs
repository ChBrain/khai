...
describe("third-place: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("third-place: manifest", () => {
  it("declares the third-place place engine", () => {
    expect(manifest.engine).toBe("third-place");
    expect(manifest.type).toBe("place");
    expect(manifest.anchor).toBe("place_third_place.md");
...
describe("third-place: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-third-place" })).toThrow();
  });
