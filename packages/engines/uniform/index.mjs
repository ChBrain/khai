// Entry point for the uniform engine. Reads the declarative `khai` manifest
...
//
// Uniform is a piece engine: the anchor is the object -- the issued garment
// itself -- and the members hang flat beneath it. One position names who is in
// it, and three processes name what it does: the issuing, the wearing, the
// taking off.
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-uniform: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
