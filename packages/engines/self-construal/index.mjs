// Entry point for the self-construal engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. independent, interdependent). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-self-construal: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
