// Entry point for the moral-identity engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. internalized, symbolic). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-moral-identity: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
