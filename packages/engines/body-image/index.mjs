// Entry point for the body-image engine. Reads the declarative `khai` manifest
...
/**
 * Assemble the instruction set for one body-image movement: the whole chain
 * from the root down to that leaf, root first, bodies only.
 *
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-body-image: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
