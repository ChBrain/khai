// Entry point for the fear engine. Reads the declarative `khai` manifest from
...
// @chbrain/khai-arch rather than re-deriving them here. Fear is multi-type: a
// process root over the three forms (process), plus the trait fearfulness
 ...
 * its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_phobia.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-fear: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
