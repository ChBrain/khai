// Entry point for the meal engine. Reads the declarative `khai` manifest from
...
//
// Meal is a process tree: a root (meal) declares eating together as an
// operation, four movement heads carry it (the table, the sharing, the order,
// the bond), and each leaf is one form. The canon owns the tree shape, so this
// loader pulls the composition chains from @chbrain/khai-arch rather than
// re-deriving them here.
//
// The four run in order within a single sitting and loop across many, since the
// bond is refreshed by repetition or fades -- so a play composes the members it
// needs rather than treating the tree as a one-way sequence.
 ...
 *
 * @param {{ leaf: string }} opts  a member file, e.g. "process_the_dividing.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-meal: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
