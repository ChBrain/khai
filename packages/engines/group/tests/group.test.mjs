// The group engine tests only what is group-specific: that the package
...
describe("group: conforms to the canon", () => {
...
describe("group: manifest", () => {
  it("declares the group engine and its anchor", () => {
    expect(manifest.engine).toBe("group");
    expect(manifest.members).toHaveLength(7);
    const root = manifest.members.find((m) => m.parent === null);
    expect(root.file).toBe("process_evolve_group.md");
  });
...
describe("group: compose()", () => {
  for (const leaf of Object.keys(chains)) {
    it(`composes ${leaf}: anchor first`, () => {
      const out = compose({ leaf });
      expect(out.trimStart().split("\n")[0]).toBe("# Process: Evolve Group");
    });
  }
