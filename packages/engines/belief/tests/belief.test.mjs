// The belief engine tests only what is belief-specific: that the package
...
// --- self-conformance: belief certifies itself against the canon ----------
describe("belief: conforms to the canon", () => {
...
// --- manifest contract ----------------------------------------------------
describe("belief: manifest", () => {
  it("declares the belief process engine", () => {
    expect(manifest.engine).toBe("belief");
    expect(manifest.type).toBe("process");
    expect(manifest.members).toHaveLength(6);
    expect(manifest.members.find((m) => m.parent === null).file).toBe("process_belief.md");
  });

  it("declares both enforceable wiring altitudes, each at its level", () => {
    // The engine owns the contract; the kit enforces it. One: the law linking
    // the root process. Two: the response linking the specific leaf settlement.
...
describe("belief: compose()", () => {
  const root = "Process: Belief";
  ...
  it("rejects an unknown response", () => {
    expect(() => compose({ leaf: "process_doubt.md" })).toThrow();
  });
