// Entry point for the belief engine. Reads the declarative `khai` manifest
...
//
// Like emotion, belief is a process tree: a root (belief) and its leaves
// (update, rationalize, deny, convert, suspend). The canon owns the tree shape,
 ...
 * deeper member carries the shallower ones upward, so composing a response
 * emits its root with it.
 *
 * @param {{ leaf: string }} opts  leaf is a member file, e.g. "process_deny.md"
...
export function compose({ leaf } = {}) {
  ...
  if (!chain) {
    ...
    throw new Error(
      `khai-engine-belief: compose() needs { leaf } to be one of [${valid}]; got ${JSON.stringify(leaf)}`,
    );
  }
