...
describe("backstage: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("backstage: manifest", () => {
  it("declares the backstage place engine", () => {
    expect(manifest.engine).toBe("backstage");
    expect(manifest.type).toBe("place");
    expect(manifest.anchor).toBe("place_backstage.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["frontstage", "outside"]);
  });
...
describe("backstage: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-region" })).toThrow();
  });
