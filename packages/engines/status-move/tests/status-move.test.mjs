...
describe("status-move: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("status-move: manifest", () => {
  it("declares the status-move position engine", () => {
    expect(manifest.engine).toBe("status-move");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_status_move.md");
...
describe("status-move: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-move" })).toThrow();
  });
