...
describe("goal: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("goal: manifest", () => {
  it("declares the goal position engine", () => {
    expect(manifest.engine).toBe("goal");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_goal.md");
...
describe("goal: compose()", () => {
  ...
  it("rejects an unknown expression", () => {
    expect(() => compose({ expression: "not-a-goal" })).toThrow();
  });
