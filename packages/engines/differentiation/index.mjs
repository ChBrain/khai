// Entry point for the differentiation engine. Reads the declarative `khai`
// manifest from package.json (the single source of truth for how this engine
// wires) and assembles the markdown positions into a ready-to-use instruction
// set.
//
...
// Differentiation is the persona-side sibling of the triangulation engine in the
// family-systems tradition: an anchor (the capacity to hold a self under relational
// pressure) over three stances -- fusion, cutoff, and the solid self.
...
/** Each expression, body only, keyed by name (e.g. fusion, cutoff, solid-self). */
 ...
 * expression: the anchor followed by the chosen stance.
 *
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-differentiation: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
