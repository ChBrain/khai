// Entry point for the worth-logic engine. Reads the declarative `khai`
...
/** Each expression, body only, keyed by name (e.g. dignity, honor, face_worth). */
...
export function compose({ expression } = {}) {
  if (!expression || !(expression in expressions)) {
    ...
    throw new Error(
      `khai-engine-worth-logic: compose() needs { expression } to be one of [${valid}]; got ${JSON.stringify(expression)}`,
    );
  }
