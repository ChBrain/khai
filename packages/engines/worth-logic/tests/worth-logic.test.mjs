...
describe("worth-logic: conforms to the canon", () => {
  it("the whole package validates (content + manifest + compose)", async () => {
    expect(flatten(await validateEnginePackage(pkgDir, { executeCompose: true }))).toEqual([]);
  });
});

describe("worth-logic: manifest", () => {
  it("declares the position engine over the three worth-logics", () => {
    expect(manifest.engine).toBe("worth-logic");
    expect(manifest.type).toBe("position");
    expect(manifest.anchor).toBe("position_worth_logic.md");
    expect(Object.keys(manifest.expressions).sort()).toEqual(["dignity", "face_worth", "honor"]);
  });
