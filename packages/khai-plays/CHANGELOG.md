...
- 1ba4ab5: The bill carried a card for a house that was never raised, registered in an
...
- b769b93: The voice layer named the tool files as the home of the coding rules. It is read
...
- 9b2e147: Register the Phoenix house on the bill: a bestiary of fire, one beast per named phenomenon of combustion. Its package and repository are named off the khai-plays-<source> convention, since the house has no public-domain author behind it and takes its source from the fire and combustion engines instead.
...
- 35d6159: Register two missing houses on the bill and re-render the README from the whole registry.

  - **Life 2 Live** (`l2`): `registry/l2.json`, programme `@chbrain/khai-plays-l2`, `repo` overridden to `ChBrain/L2` (the house repository is `L2`, not `khai-plays-l2`).
  - **Misfits** (`misfits`): `registry/misfits.json`, both `package` (`@chbrain/khai-misfits`) and `repo` (`ChBrain/khai-misfits`) overridden, since this house does not follow the `khai-plays-<slug>` naming.

## 0.0.14

### Patch Changes

- 0baf77d: Register the Storm house in the play registry (the bill): add `registry/storm.json`
  (Theodor Storm, `@chbrain/khai-plays-storm`) and regenerate `README.md` from the
  whole registry. The website reads the card and pulls the programme to render the
  house.
...
- 1be4233: Name the cast in the Director position (doctrine copy). The position now encodes
  the separation of two stances (an immersed cast that embodies the scene, the
  producer of the run; the Director outside it, reading and redirecting) as the
  discipline that makes behaviour evidence and not the Director's echo. The Director
  authors the production (the reading), never the lines; a captured or handed-over
  run is the cast's work, selected and sequenced, never written by the Director.

  Also adds "tune the pitch" to the redirect idioms: the Director tunes the run to a
  dominant key, or re-pitches it (the pitch element type lands in khai-arch).
...
- 3d7f100: Rework the four house blurbs to be true to each canon and free of stale
  specifics. Drop the repetitive "The productions of X" lead (the card already
  shows the house name); remove Grimm's hardcoded play count (it drifts as plays
  are added); and re-flavor HCA and Grimm in their canon rather than their voice
  or translation — HCA: longing, cruelty, and grace (now spanning the trickster
  tales through the Little Mermaid); Grimm: oaths, taboos, and transformations run
  to their exact and merciless end. Applied to `registry/*.json` and the README
  mirror.
- 65c6c7b: Cast the Director in the chain's reference company: `position_director` (the
  hand that stages a score into a performance), `persona_director` (Thespis, the
  reference Director; each house casts its own), and `plan_stage_the_score` (the
  Director's render mandate). Completes the blueprint cast Order 1 builds the
  skill against.
