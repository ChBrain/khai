...
| 39  | **mania**          | elevated/expansive/irritable activation episode; grandiosity, reduced sleep need | Depue & Iacono (BFS); S. L. Johnson (BAS dysregulation); Akiskal (hyperthymic temperament)                                 | `joy` (appraisal node); `flow` (absorption); homonym w/ `desire/obsessive`'s Lee love-style label                                            | Engine (process)  | building        |
| 40  | **social-anxiety** | fear of negative evaluation — anticipatory / performance / post-event            | Clark & Wells (1995); Rapee & Heimberg (1997); Leary (sociometer); Hofmann                                                 | `fear/phobia` (conditioned object-fear, not evaluation); `embarrassment` (acute, not anticipatory-chronic)                                   | Engine (process)  | building        |
| 41  | **dissociation**   | disconnection from thought/feeling/memory/identity/surroundings under overwhelm  | Putnam (discrete states); van der Hart et al. (structural dissociation); Bernstein & Putnam (DES); Freyd (betrayal trauma) | `the-unconscious` (repression, not structural split); `ptsd/estrangement` (numbing, not splitting); `defense/immature` (category, not depth) | Engine (process)  | building        |
...
| --- | -------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ---------- | -------- |
| 50  | **depression** (MDE)                   | sad mood + anhedonia + neurovegetative + worthlessness, as a syndrome  | `sadness`(despair) + **anhedonia** + `rumination` + `self-esteem` + `body`      | Beck; Seligman; Nesse. Highest-value composite in the set — MDD is the most-requested clinical persona | #38        | proposed |
...
| `emotional`       | neither — both qualify                                | `forgiveness/process_emotional.md`             | `emotional_forgiveness`  | Worthington's own term                                                                                                                               | 1     | building        |
...
| `self`            | `bias` keeps it — its family roots are bare by design | `forgiveness/process_self.md`                  | `self_forgiveness`       | the field's own term                                                                                                                                 | 0     | building        |
...
A supplemental research pass (August 2026) surfaced several high-value, unmodeled phenomena across group dynamics, narrative reception, and clinical architecture that are distinct from the existing inventory.
...
| 57  | **deindividuation**   | loss of self-awareness and evaluation apprehension in group settings → disinhibited behavior | Festinger/Pepitone/Newcomb (1952); Zimbardo (1969); Reicher (SIDE model)                       | `conformity` (yielding to norms); `social-identity` (categorization). Deindividuation is the _anonymity-driven disinhibition_, not just group alignment          | Engine (process)  | proposed |
| 58  | **delusion**          | fixed, false beliefs resistant to contradictory evidence (persecutory, grandiose, somatic)   | Jaspers (primary vs secondary); Freeman & Garety (cognitive model of persecutory delusions)    | `belief-in-a-just-world` (a specific worldview); `superstition` (magical thinking, not a fixed break from reality)                                               | Engine (position) | proposed |
| 59  | **illness-anxiety**   | preoccupation with having or acquiring a serious undiagnosed medical illness                 | Warwick & Salkovskis (cognitive-behavioral model of hypochondriasis); DSM-5                    | `anxiety` (general threat anticipation); `body` (interoception — illness-anxiety is the _catastrophic misinterpretation_ of those signals)                       | Engine (process)  | proposed |
...
| 63  | **bottleneck**        | the slowest or most constrained stage dictates the throughput of the entire process         | Goldratt (Theory of Constraints, 1984)     | `stress` (psychological load); `scarcity` (resource lack). A bottleneck is a structural constraint on _flow_, not an absolute lack                      | Engine (process) | proposed |
| 64  | **normal-accident**   | complex, tightly-coupled systems inevitably experience unpredictable, cascading failures    | Perrow (1984); Dekker (drift into failure) | `mistake` / `bias` (human error). The normal-accident is a _property of the system architecture_, not operator error                                    | Engine (process) | proposed |
| 65  | **path-dependence**   | historical choices constrain future possibilities (lock-in), even when suboptimal           | David (1985 - QWERTY); Arthur (1989)       | `habit` (individual automaticity); `tradition` (cultural transmission). Path-dependence is the _structural switching cost_ in a system                  | Engine (process) | proposed |
| 66  | **leverage-point**    | specific places in a complex system where a small shift produces large systemic changes     | Meadows (1999)                             | `catalyst` (chemical/generic). Leverage points are specific systemic interventions (e.g., feedback loops, information flows)                            | Engine (process) | proposed |
| 67  | **cascading-failure** | the failure of one node triggers successive failures down the line                          | Watts (2002); Barabási (Network Science)   | `domino-effect` (colloquial). Cascading failure is the specific networked propagation of overload                                                       | Engine (process) | proposed |
...
| #   | Candidate                   | Phenomenon                                                                                  | Warrant                                   | Nearest / boundary                                                                                                                               | E/C               | Status   |
| --- | --------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | -------- |
| 68  | **bureaucracy**             | the structuring of power through impersonal rules, hierarchy, and specialized jurisdiction  | Weber (1922)                              | `organization` (generic grouping); `power` (abstract). Bureaucracy is the specific _impersonal, rule-bound_ machinery of power                   | Engine (process)  | proposed |
| 69  | **tragedy-of-the-commons**  | individual rational behavior over-consumes a shared resource, leading to collective ruin    | Hardin (1968); Ostrom (1990)              | `scarcity` (resource lack); `selfishness`. The tragedy is a _structural trap_ inherent to shared, unregulated pools                              | Engine (process)  | proposed |
| 70  | **principal-agent-problem** | the conflict in priorities between a person or group and the representative acting for them | Ross (1973); Jensen & Meckling (1976)     | `trust` (interpersonal); `betrayal`. This is a _structural misalignment of incentives_ when agency is delegated                                  | Engine (position) | proposed |
| 71  | **symbiosis**               | a persistent, close biological or structural interaction between two different organisms    | De Bary (1879); Margulis (Endosymbiosis)  | `cooperation` (intentional behavior); `attachment`. Symbiosis is an _obligate or mutually-shaping structural reliance_                           | Engine (process)  | proposed |
| 72  | **precedent**               | a past decision serving as an authoritative rule or pattern for future, similar cases       | Stare decisis (Legal Theory)              | `tradition` (cultural inertia); `habit` (individual). Precedent is an _explicitly cited, binding rule_ generated from a past event               | Engine (plan)     | proposed |
| 73  | **hyperreality**            | the inability to distinguish reality from a simulation of reality                           | Baudrillard (1981); Eco                   | `delusion` (clinical). Hyperreality is a _cultural/media condition_ where the copy replaces the original                                         | Engine (process)  | proposed |
| 74  | **framing**                 | the presentation of information that influences how people process and interpret it         | Goffman (1974); Kahneman & Tversky (1981) | `bias` (cognitive error); `propaganda` (malicious intent). Framing is the _unavoidable structural context_ that sets the terms of interpretation | Engine (process)  | proposed |
...
| #   | Candidate                   | Phenomenon                                                                                   | Warrant                                             | Nearest / boundary                                                                                                                                      | E/C               | Status   |
| --- | --------------------------- | -------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | -------- |
| 75  | **friction**                | the degradation of a plan when executed in reality due to accumulated, unpredictable factors | Clausewitz (_On War_)                               | `mistake` (human error); `decay` (material entropy). Friction is the specific _resistance of reality_ to a theoretical plan                             | Engine (plan)     | proposed |
| 76  | **deterrence**              | the prevention of an action by the credible threat of unacceptable counter-action            | Schelling (MAD / Game Theory)                       | `warning` (communication); `fear` (emotion). Deterrence is a _structural stalemate_ built on mutually understood capabilities                           | Engine (position) | proposed |
| 77  | **attrition**               | winning a conflict by gradually wearing down the opponent's capacity and will to fight       | Strategy / Military Theory                          | `wear` (object decay); `stress` (psychological). Attrition is a _strategic posture_ choosing endurance over decisive maneuver                           | Engine (process)  | proposed |
| 78  | **shibboleth**              | a linguistic or cultural marker that unconsciously reveals in-group/out-group status         | Linguistics / Sociology                             | `stigma` (a visible mark of shame); `secret` (hidden knowledge). A shibboleth is an _unintentional password_ encoded in behavior                        | Engine (piece)    | proposed |
...
| 80  | **collective-memory**       | how a society actively constructs, forgets, and enforces a shared version of its past        | Halbwachs (1925)                                    | `memory` (individual recall); `tradition` (inherited practice). Collective memory is _socially negotiated historiography_ serving present needs         | Engine (process)  | proposed |
| 81  | **orthodoxy**               | the systemic definition and enforcement of ideological boundaries to protect a core system   | Theology / Sociology                                | `conformity` (yielding to a group); `ideology`. Orthodoxy focuses on the _boundary policing and heresy-hunting_ mechanisms                              | Engine (process)  | proposed |
